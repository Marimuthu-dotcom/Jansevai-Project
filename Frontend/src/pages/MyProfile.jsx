...
function MyProfile() {
  ...
  const { user , token, myComplaints ,loading,activity} = useContext(AuthContext);
  ...
  // Logout row click — dropdown திறக்கும்
...
const getStatus = (status) => {
  ...
  return (
    <div className={styles.mainContainer}>
                ...
                {SETTINGS.map((s) => (
                  <div key={s.id} className={styles.settingRow}>
                    ...
                    {s.type === "toggle" ? (
                      ...
                      <div className={styles.settingArrow} onClick={s.name === "Logout" ? handleLogoutClick : undefined}>
      ...
      {/* ✅ Old dropdown remove பண்ணு — இதை போடு */}
{showLogoutConfirm && (
  <div className={styles.logoutOverlay} onClick={handleCancelLogout}>
    <div
      className={`${styles.logoutDropdown} ${confirmClosing ? styles.dropdownOut : styles.dropdownIn}`}
      onClick={(e) => e.stopPropagation()}
    >
      {/* Icon */}
      <div className={styles.logoutIcon}>
        <svg viewBox="0 0 24 24" fill="none" stroke="#e02020" strokeWidth="2">
      ...
      <p className={styles.logoutMsg}>Are you sure you want to sign out of your account?</p>

      <div className={styles.logoutBtnRow}>
        <button className={styles.cancelLogoutBtn} onClick={handleCancelLogout}>
          Cancel
        </button>
        <button className={styles.confirmLogoutBtn} onClick={handleConfirmLogout}>
          Logout
        </button>
      </div>
    </div>
