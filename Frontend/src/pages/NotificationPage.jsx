...
const TABS = ["All", "Unread", "Complaint", "Support", "Comment"];
...
function NotificationPage() {
  ...
  const handleMarkRead = async (id, is_read) => {
    if (is_read) 
      return; // already read — skip API call
    try {
      await api.put(
        `/api/auth/${id}/read`);
  ...
  switch (tab) {
    ...
    case "Complaint":
      return notifications.filter(
        (n) => n.type?.toLowerCase() === "complaint"
      ).length;

    case "Support":
      return notifications.filter(
        (n) => n.type?.toLowerCase() === "support"
      ).length;
      ...
      <div className={styles.tabBar}>
        {TABS.map((tab) => (
          ...
          >
            {tab}
            {getTabCount(tab) > 0 && (
              <span className={styles.tabBadge}>{getTabCount(tab)}</span>
            )}
          </button>
        ))}
      </div>
