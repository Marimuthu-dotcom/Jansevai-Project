...
function ComplaintDetails() {
  ...
  const isOwner = complaint?.user_email === user?.email; // Check if the logged-in user is the owner of the complaint
  ...
  useEffect(() => {
    ...
    const fetchComments = async () => {
      try {
        const res = await api.get(`/api/auth/get-comments/${id}`);
  ...
  useEffect(() => {
    ...
    const fetchSupporters = async () => {
      try {
        const res = await api.get(`/api/auth/supporters/${id}`);
  ...
  const handleSupport = async () => {
    if (isOwner)
    {
    setWarningMsg("You cannot support your own complaint.");
    setTimeout(() => setWarningMsg(""), 3000);
    return; // ← API call பண்ணாதே
  }
  ...
  const handleStatusChange = async (newStatus) => {
    if (!isOwner)
    {
    setWarningMsg("You cannot allowed to update the status of the complaint.");
    setTimeout(() => setWarningMsg(""), 3000);
    return; // ← API call பண்ணாதே
  }
...
const hiddenCount     = comments.length - 2;
  ...
  return (
    <div className={styles.mainContainer}>
      {/* ✅ Warning Toast — warningMsg இருந்தா காட்டும் */}
    {warningMsg && (
      <WarningToast
        message={warningMsg}
        ...
        <div className={styles.rightCol}>
          ...
          <div className={styles.rightCard}>
            ...
            <button
              ...
              title={isOwner ? "You cannot support your own complaint" : ""}
            >
              <ThumbsUp size={15} />
              {supportLoading  ? "Updating...": supported ? "Supported ✓" : isOwner ? "Your Complaint" : "Support This"}
            </button>
          </div>
            ...
            <div className={styles.statusBtns}>
              {currentStatus === "Pending" && (
                <button
                  ...
                  title={!isOwner ? "You cannot allwoed to Update the status of the complaint" : ""}
                >
              ...
              {currentStatus === "In Progress" && (
                <button
                  ...
                  disabled={statusLoading}
                  title={!isOwner ? "You cannot Update the status the complaint" : ""}
                >
